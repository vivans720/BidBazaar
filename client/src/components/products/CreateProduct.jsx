...
const CreateProduct = () => {
  ...
  const uploadImages = async (files) => {
    ...
    if (files.length > 1) {
      ...
      try {
        ...
        throw new Error(
          `Failed to upload images: ${err.response?.data?.error || err.message}`
        );
      }
    } else {
      ...
      try {
        ...
        throw new Error(
          `Failed to upload image ${files[0].name}: ${
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.error || err.message || "Error creating product";
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-primary-100 mt-1">
                Create a new product to auction on BidBazaar
              </p>
            </div>
            <Link
              to="/dashboard"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`block w-full px-3 py-2 border ${
                          submitAttempted &&
                          (formData.duration < 1 || formData.duration > 60)
                            ? "border-red-300"
                            : "border-gray-300"
                        } rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm`}
                      />
