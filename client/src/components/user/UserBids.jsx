  ...
  const fetchUserBids = async () => {
    try {
      ...
      if (response.data.length > 0) {
        ...
        const lostBids = response.data.filter(
          (b) => b.status === "lost"
        ).length;
      ...
      const errorMessage =
        err.response?.data?.error || "Error fetching your bids";
    ...
    if (bid.status === "won") {
      ...
      return (
        <div className="text-gray-500 text-xs mt-1">
          Sold for {formatPrice(winningAmount || 0)}
        </div>
      );
      ...
      <div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-md p-8 text-center">
          ...
          <p className="text-gray-500 mb-6 max-w-md mx-auto">
            You haven't placed any bids yet. Start bidding on items to see your
            bid history here.
          </p>
          <Link
            to="/products"
            className="inline-flex items-center px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-md shadow-sm transition-colors"
          >
            Browse Auctions
          </Link>
        </div>
    ...
    <div className="animate-fadeIn">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleRefresh}
              disabled={refreshing}
              className={`inline-flex items-center px-3 py-2 bg-white border border-gray-200 text-gray-700 text-sm font-medium rounded-md shadow-sm hover:bg-gray-50 transition-all ${
          ...
          <div className="h-2 bg-blue-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${
                  stats.totalBids
                    ? (stats.activeBids / stats.totalBids) * 100
                    : 0
                }%`,
              }}
            ></div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="h-2 bg-green-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${
                  stats.totalBids ? (stats.wonBids / stats.totalBids) * 100 : 0
                }%`,
              }}
            ></div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="h-2 bg-red-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${
                  stats.totalBids ? (stats.lostBids / stats.totalBids) * 100 : 0
                }%`,
              }}
            ></div>
          </div>
      ...
      {filteredBids.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 text-center">
          ...
          <p className="text-gray-500 mb-6 max-w-md mx-auto">
            No bids match your current filter selection. Try changing your
            filter or browse all bids.
          </p>
          <button
            onClick={() => setFilter("all")}
        ...
        <div className="space-y-4">
          {filteredBids.map((bid, index) => (
            <div
              key={bid._id}
              className={`bg-white rounded-xl border shadow-sm overflow-hidden hover:shadow-md transition-all duration-300 ${getStatusBgColor(
                bid.status
              )} animate-fadeIn`}
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                        ...
                        <div
                          className={`px-3 py-1 inline-flex items-center text-xs font-semibold rounded-full ${getBadgeColor(
                            bid.status
                          )}`}
                        >
                          {getStatusIcon(bid)}
                  ...
                  <div className="flex flex-col items-end mt-2 md:mt-0">
                    ...
                    <div className="mt-3 flex flex-col gap-2">
                      <Link
                        to={`/products/${bid.product?._id}`}
                        className="inline-flex items-center px-3 py-1.5 bg-white border border-gray-200 text-primary-600 text-sm font-medium rounded-md hover:bg-primary-50 hover:shadow-sm transition-all duration-200"
                      >
                        <EyeIcon className="h-4 w-4 mr-1.5" />
                        View Product
                      </Link>

                      {/* Rating Button for Won Auctions */}
                      {bid.status === "won" &&
                        bid.product?.status === "ended" && (
                          <button
                            onClick={() =>
                              navigate(
                                `/feedback/submit/${bid.product._id}?bidId=${bid._id}`
                              )
                            }
                            className="inline-flex items-center px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-white text-sm font-medium rounded-md shadow-sm transition-all duration-200 hover:shadow-md"
                          >
